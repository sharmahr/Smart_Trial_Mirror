...
while True:
    ...
    if grabbed == False:
        print('Failed to capture frame from camera. Check the camera index \n')
        break
    ...
    for (x, y, w, h) in faces:
        ...
        mask_resize = cv2.resize(mask, (96, 96), interpolation=cv2.INTER_AREA)
        ...
        mask_resize[int(points[9][1]):int(points[9][1])+sunglass_height, int(points[9][0]):int(points[9][0])+sunglass_width,:][transparent_region] = sunglass_resized[:,:,:3][transparent_region]

        # Resize the face_resized_color image back to its original shape
        frame[y:y+h, x:x+w] = cv2.resize(face_resized_color, original_shape, interpolation = cv2.INTER_CUBIC)
        mask[y:y+h, x:x+w] = cv2.resize(mask_resize, original_shape, interpolation = cv2.INTER_CUBIC)
